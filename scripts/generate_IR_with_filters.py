...
if __name__ == "__main__":
    # Apply frequency dependent wall absorption coefficients to simulate realistic wall/ceiling/floor materials
    ...
    room_param = rp.RoomParameters(
        ...
        # wall_coeffs=6*[0.5],
        wall_coeffs = wall_materials,
        # If true, use frequency dependant absorption coeffients based on real materials.
        beta_direct=freq_dep_abs_coeff
    )

    if freq_dep_abs_coeff:
        receiver_channels, pos_rcv, fs, bit_depth = fdac.generate_RIR_freq_dep_walls(wall_materials)
    else:
        receiver_channels, pos_rcv, fs, bit_depth = generate_RIR(room_param)
